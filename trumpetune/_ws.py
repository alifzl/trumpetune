# https://github.com/imachug/gevent-ws
...
class WebSocket:
    ...
    def _send_frame(self, opcode, data):
        for i in range(0, len(data), SEND_PACKET_SIZE):
            ...
            header = bytes(
                [(opcode if i == 0 else 0) | (fin << 7), min(len(part), 126)]
            )
            if len(part) >= 126:
...
class WebSocketHandler(WSGIHandler):
    def handle_one_response(self):
        ...
        http_connection = [
            s.strip().lower()
            for s in self.environ.get("HTTP_CONNECTION", "").split(",")
        ]
        if (
            "upgrade" not in http_connection
            or self.environ.get("HTTP_UPGRADE", "").lower() != "websocket"
        ):
            # Not my problem
